...
static int cbm2cartridge_reset; /* (resource) hardreset system after cart was
                                   attached/detached */
...
static export_resource_t export_res08 = {"RAM", 0,    CBM2_CART_BLK08,
                                         NULL,  NULL, 0};
static export_resource_t export_res1 = {"RAM", 0,    CBM2_CART_BLK1,
                                        NULL,  NULL, 0};
static export_resource_t export_res2 = {"RAM", 0,    CBM2_CART_BLK2,
                                        NULL,  NULL, 0};
static export_resource_t export_res4 = {"RAM", 0,    CBM2_CART_BLK4,
                                        NULL,  NULL, 0};
static export_resource_t export_res6 = {"RAM", 0,    CBM2_CART_BLK6,
                                        NULL,  NULL, 0};
...
uint8_t *ultimax_romh_phi1_ptr(uint16_t addr) { return romh_banks; }

uint8_t *ultimax_romh_phi2_ptr(uint16_t addr) { return romh_banks; }
...
static int try_cartridge_attach(int type, const char *filename) {
  if (filename) {
    if (util_file_exists(filename)) {
      ...
      DBG(("try_cartridge_attach: cartridge_file does not exist: '%s'",
           filename));
    }
  }
...
static const resource_int_t resources_int[] = {
    {"CartridgeReset", 1, RES_EVENT_NO, NULL, &cbm2cartridge_reset,
    ...
    {"CartridgeType", CARTRIDGE_NONE, RES_EVENT_STRICT,
     ...
     NULL},
    {"Ram08", 0, RES_EVENT_NO, NULL, &cart08_ram, set_cart08_ram, NULL},
    {"Ram1", 0, RES_EVENT_NO, NULL, &cart1_ram, set_cart1_ram, NULL},
    {"Ram2", 0, RES_EVENT_NO, NULL, &cart2_ram, set_cart2_ram, NULL},
    {"Ram4", 0, RES_EVENT_NO, NULL, &cart4_ram, set_cart4_ram, NULL},
    {"Ram6", 0, RES_EVENT_NO, NULL, &cart6_ram, set_cart6_ram, NULL},
    {"RamC", 0, RES_EVENT_NO, NULL, &cartC_ram, set_cartC_ram, NULL},
...
*/
static void cart_attach_from_snapshot(int type) { cbm2cart_type = type; }
...
#endif
#if 1
  generic_detach(CARTRIDGE_CBM2_GENERIC_C1 | CARTRIDGE_CBM2_GENERIC_C2 |
                 CARTRIDGE_CBM2_GENERIC_C4 | CARTRIDGE_CBM2_GENERIC_C6);
  /* add extra cartridges here */
#endif
...
int cartridge_snapshot_write_modules(struct snapshot_s *s) {
  ...
  m = snapshot_module_create(s, SNAP_MODULE_NAME, CBM2CART_DUMP_VER_MAJOR,
                             CBM2CART_DUMP_VER_MINOR);
  if (m == NULL) {
    return -1;
  }
...
int cartridge_snapshot_read_modules(struct snapshot_s *s) {
  ...
  if ((vmajor != CBM2CART_DUMP_VER_MAJOR) ||
      (vminor != CBM2CART_DUMP_VER_MINOR)) {
    goto fail;
  }
