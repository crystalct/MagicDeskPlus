...
/*
    ...
    - all cards *except* those in the "Main Slot" should:
      - maintain a resource (preferably XYZCartridgeEnabled) that tells
        wether said cart is "inserted" into our virtual "expansion port
   expander".
...
static cartridge_info_t cartlist[] = {
    ...
    {CARTRIDGE_VIC20_NAME_BEHRBONZ, CARTRIDGE_VIC20_BEHRBONZ,
     CARTRIDGE_GROUP_UTIL},
    {CARTRIDGE_VIC20_NAME_FINAL_EXPANSION, CARTRIDGE_VIC20_FINAL_EXPANSION,
     CARTRIDGE_GROUP_UTIL},
    {CARTRIDGE_VIC20_NAME_FP, CARTRIDGE_VIC20_FP, CARTRIDGE_GROUP_UTIL},
    {CARTRIDGE_VIC20_NAME_MEGACART, CARTRIDGE_VIC20_MEGACART,
     CARTRIDGE_GROUP_UTIL},
    {CARTRIDGE_VIC20_NAME_MIKRO_ASSEMBLER, CARTRIDGE_VIC20_MIKRO_ASSEMBLER,
     CARTRIDGE_GROUP_UTIL},
    {CARTRIDGE_VIC20_NAME_MINIMON, CARTRIDGE_VIC20_MINIMON,
     CARTRIDGE_GROUP_UTIL}, /* slot 0 */
    {CARTRIDGE_VIC20_NAME_RABBIT, CARTRIDGE_VIC20_RABBIT, CARTRIDGE_GROUP_UTIL},
    {CARTRIDGE_VIC20_NAME_SUPEREXPANDER, CARTRIDGE_VIC20_SUPEREXPANDER,
     CARTRIDGE_GROUP_UTIL},
    {CARTRIDGE_VIC20_NAME_UM, CARTRIDGE_VIC20_UM, CARTRIDGE_GROUP_UTIL},
    {CARTRIDGE_VIC20_NAME_WRITE_NOW, CARTRIDGE_VIC20_WRITE_NOW,
     CARTRIDGE_GROUP_UTIL},
...
static const resource_int_t resources_int[] = {
    {"CartridgeType", CARTRIDGE_NONE, RES_EVENT_STRICT,
     ...
     NULL},
    {"CartridgeReset", 1, RES_EVENT_NO, NULL, &vic20cartridge_reset,
...
int cartridge_resources_init(void) {
  if (resources_register_int(resources_int) < 0 ||
      ...
      generic_resources_init() < 0 || finalexpansion_resources_init() < 0 ||
      vic_fp_resources_init() < 0 || vic_um_resources_init() < 0 ||
      megacart_resources_init() < 0 || minimon_resources_init() < 0
#ifdef HAVE_RAWNET
      || ethernetcart_resources_init() < 0
#endif
      || aciacart_resources_init() < 0 || digimax_resources_init() < 0 ||
      ds12c887rtc_resources_init() < 0 ||
      sfx_soundexpander_resources_init() < 0 ||
      sfx_soundsampler_resources_init() < 0 || ioramcart_resources_init() < 0 ||
      georam_resources_init() < 0 || debugcart_resources_init() < 0) {
    return -1;
  }
  return 0;
}
...
static const cmdline_option_t cmdline_options[] = {
    ...
    {"-cartcrt", CALL_FUNCTION, CMDLINE_ATTRIB_NEED_ARGS,
     attach_cartridge_cmdline, (void *)CARTRIDGE_CRT, NULL, NULL, "<Name>",
    ...
    {"-cartfe", CALL_FUNCTION, CMDLINE_ATTRIB_NEED_ARGS,
     attach_cartridge_cmdline, (void *)CARTRIDGE_VIC20_FINAL_EXPANSION, NULL,
     NULL, "<Name>", "Specify Final Expansion extension ROM name"},
    {"-ultimem", CALL_FUNCTION, CMDLINE_ATTRIB_NEED_ARGS,
     attach_cartridge_cmdline, (void *)CARTRIDGE_VIC20_UM, NULL, NULL, "<Name>",
     "Specify UltiMem extension ROM name"},
    {"-cartfp", CALL_FUNCTION, CMDLINE_ATTRIB_NEED_ARGS,
     attach_cartridge_cmdline, (void *)CARTRIDGE_VIC20_FP, NULL, NULL, "<Name>",
    ...
    {"-cartse", CALL_FUNCTION, CMDLINE_ATTRIB_NEED_ARGS,
     attach_cartridge_cmdline, (void *)CARTRIDGE_VIC20_SUPEREXPANDER, NULL,
     NULL, "<Name>",
     "Specify " CARTRIDGE_VIC20_NAME_SUPEREXPANDER " cartridge ROM name"},
    {"-cartma", CALL_FUNCTION, CMDLINE_ATTRIB_NEED_ARGS,
     attach_cartridge_cmdline, (void *)CARTRIDGE_VIC20_MIKRO_ASSEMBLER, NULL,
     NULL, "<Name>",
     "Specify " CARTRIDGE_VIC20_NAME_MIKRO_ASSEMBLER " cartridge ROM name"},
    ...
    /* no cartridge */
    {"+cart", CALL_FUNCTION, CMDLINE_ATTRIB_NONE, detach_cartridge_cmdline,
     NULL, NULL, NULL, NULL, "Disable default cartridge"},
    CMDLINE_LIST_END};
...
int cartridge_cmdline_options_init(void) {
  ...
  if (cmdline_register_options(cmdline_options) < 0 ||
      ...
      vic_fp_cmdline_options_init() < 0 || vic_um_cmdline_options_init() < 0 ||
...
int cartridge_attach_image(int type, const char *filename) {
  ...
  DBG(("cartridge_attach_image type '%d'(0x%04x) name: '%s'", type,
       (unsigned)type, filename));
  /*
      if (filename == NULL) {
          return -1;
      }
  */
  ...
  log_message(LOG_DEFAULT, "Attached cartridge type %d, file=`%s'.", type,
              filename);
#if 0
  ...
  DBG(("CART: attach RAW ID: %d carttype: %d vic20cart_type:%d", cartid,
       carttype, vic20cart_type));
  ...
  lib_free(rawcart);
  log_message(LOG_DEFAULT, "CART: attached '%s' as ID %d.", abs_filename,
              carttype);
  lib_free(abs_filename);
  return 0;
...
int cartridge_attach_add_image(int type, const char *filename) {
  ...
  DBG(("cartridge_attach_add_image type '%d'(0x%04x) name: '%s'", type,
       (unsigned)type, filename));
  /*
      if (filename == NULL) {
          return -1;
  ...
  */
  ...
  /* NOTE: when we use more than one .bin file, we cant set it as default
   * cartridge */
  util_string_set(&cartfile, NULL);
  cartridge_attach(vic20cart_type, NULL);

  DBG(("CART: cartridge_attach_add_image type: %d ID: %d done.", type,
       carttype));
  lib_free(rawcart);
  log_message(LOG_DEFAULT, "CART: attached '%s' as ID %d.", abs_filename,
              carttype);
...
void cartridge_detach_image(int type) {
  DBG(("CART: cartridge_detach_image vic20cart_type:%d type:%d", vic20cart_type,
       type));
...
int cartridge_bin_save(int type, const char *filename) {
  ...
  log_error(LOG_DEFAULT,
            "Failed saving binary cartridge image for cartridge ID %d.", type);
  return -1;
}
...
int cartridge_get_id(int slot) {
  ...
  DBG(("cartridge_get_id(slot:%d): %d %d type:%d", slot, cartridge_type,
       vic20cart_type, type));
  return type;
}
...
char *cartridge_get_filename_by_slot(int slot) {
  if (vic20cart_type == CARTRIDGE_VIC20_GENERIC) {
    /* special case handling for the multiple file generic type */
    /* return generic_get_file_name((uint16_t)addr); */
    log_warning(LOG_DEFAULT, "FIXME: cartridge_get_filename_by_slot not "
                             "implemented for generic type");
  }
...
char *cartridge_get_secondary_filename_by_slot(int slot) {
  log_error(
      LOG_DEFAULT,
      "FIXME: cartridge_get_secondary_filename_by_slot not implemented yet");
  return NULL;
}
...
int vic20cart_snapshot_write_module(snapshot_t *s) {
  ...
  m = snapshot_module_create(s, SNAP_MODULE_NAME, VIC20CART_DUMP_VER_MAJOR,
