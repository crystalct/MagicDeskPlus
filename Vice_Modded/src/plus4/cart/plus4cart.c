...
static int plus4cartridge_reset; /* (resource) hardreset system after cart was
                                    attached/detached */

/* defaults */
static char *cartridge_file = NULL; /* (resource) file name */
static int cartridge_type =
    CARTRIDGE_NONE; /* (resource) is == CARTRIDGE_CRT (0) if CRT file */

/* actually in use */
static char *cartfile = NULL; /* file name */
static int plus4cart_type =
    CARTRIDGE_NONE; /* is == CARTRIDGE_CRT (0) if CRT file */
static int crttype =
    CARTRIDGE_NONE; /* contains CRT ID if plus4cart_type == 0 */

static int mem_cartridge_type =
    CARTRIDGE_NONE; /* Type of the cartridge attached. */
...
static cartridge_info_t cartlist[] = {
    ...
    {CARTRIDGE_PLUS4_NAME_JACINT1MB, CARTRIDGE_PLUS4_JACINT1MB,
    ...
    {CARTRIDGE_PLUS4_NAME_SPEEDY, CARTRIDGE_PLUS4_SPEEDY,
     CARTRIDGE_GROUP_FREEZER},

    {NULL, 0, 0}};

cartridge_info_t *cartridge_get_info_list(void) { return &cartlist[0]; }
...
/* FIXME: slot arg is ignored right now.
   this should return a pointer to a filename, or NULL
*/
char *cartridge_get_secondary_filename_by_slot(int slot) { return NULL; }
...
static const cmdline_option_t cmdline_options[] = {
    ...
    {"-cartreset", SET_RESOURCE, CMDLINE_ATTRIB_NONE, NULL, NULL,
     "CartridgeReset", (void *)1, NULL,
     "Reset machine if a cartridge is attached or detached"},
    {"+cartreset", SET_RESOURCE, CMDLINE_ATTRIB_NONE, NULL, NULL,
     "CartridgeReset", (void *)0, NULL,
     "Do not reset machine if a cartridge is attached or detached"},
    /* smart attach */
    {"-cart", CALL_FUNCTION, CMDLINE_ATTRIB_NEED_ARGS, cart_attach_cmdline,
     (void *)CARTRIDGE_PLUS4_DETECT, NULL, NULL, "<Name>",
    ...
    {"-cartmagic", CALL_FUNCTION, CMDLINE_ATTRIB_NEED_ARGS, cart_attach_cmdline,
     (void *)CARTRIDGE_PLUS4_MAGIC, NULL, NULL, "<Name>",
     "Attach 128kiB/256kiB/512kiB/1MiB/2MiB " CARTRIDGE_PLUS4_NAME_MAGIC
     " image"},
    {"-cartmulti", CALL_FUNCTION, CMDLINE_ATTRIB_NEED_ARGS, cart_attach_cmdline,
     (void *)CARTRIDGE_PLUS4_MULTI, NULL, NULL, "<Name>",
     "Attach 1MiB/2MiB " CARTRIDGE_PLUS4_NAME_MULTI " image"},
    {"-cartspeedy", CALL_FUNCTION, CMDLINE_ATTRIB_NEED_ARGS,
     cart_attach_cmdline, (void *)CARTRIDGE_PLUS4_SPEEDY, NULL, NULL, "<Name>",
     "Attach 8KiB " CARTRIDGE_PLUS4_NAME_SPEEDY " image"},

    /* no cartridge */
    {"+cart", CALL_FUNCTION, CMDLINE_ATTRIB_NONE, cart_attach_cmdline, NULL,
     NULL, NULL, NULL, "Disable default cartridge"},
    CMDLINE_LIST_END};
...
/*
    we have 3 resources for the main cart that may be changed in arbitrary
   order:
    ...
    the following functions try to deal with this in a hopefully sane way...
   however, do _NOT_ change the used resources from the (G)UI directly. (used
   the set_default function instead)
*/
...
static const resource_int_t resources_int[] = {
    {"CartridgeReset", 1, RES_EVENT_NO, NULL, &plus4cartridge_reset,
     set_cartridge_reset, NULL},
    {"CartridgeType", CARTRIDGE_NONE, RES_EVENT_STRICT,
     (resource_value_t)CARTRIDGE_NONE, &cartridge_type, set_cartridge_type,
     NULL},
    RESOURCE_INT_LIST_END};

static const resource_string_t resources_string[] = {
    {"CartridgeFile", "", RES_EVENT_NO, NULL, &cartridge_file,
...
uint8_t plus4cart_kernal_read(uint16_t addr) {
  ...
  DBGRW(("plus4cart_kernal_read mem_cartridge_type: %04x addr: %04x",
         (unsigned)mem_cartridge_type, addr));
...
uint8_t plus4cart_c1lo_read(uint16_t addr) {
  DBGRW(("plus4cart_c1lo_read mem_cartridge_type: %04x addr: %04x",
         (unsigned)mem_cartridge_type, addr));
  if (CARTRIDGE_PLUS4_IS_GENERIC(mem_cartridge_type)) {
    if ((mem_cartridge_type & CARTRIDGE_PLUS4_GENERIC_TYPE_MASK) &
        CARTRIDGE_PLUS4_GENERIC_C1LO) {
...
uint8_t plus4cart_c1hi_read(uint16_t addr) {
  DBGRW(("plus4cart_c1hi_read mem_cartridge_type: %04x addr: %04x",
         (unsigned)mem_cartridge_type, addr));
  if (CARTRIDGE_PLUS4_IS_GENERIC(mem_cartridge_type)) {
    if ((mem_cartridge_type & CARTRIDGE_PLUS4_GENERIC_TYPE_MASK) &
        CARTRIDGE_PLUS4_GENERIC_C1HI) {
...
*/
static void cart_attach_from_snapshot(int type) { plus4cart_type = type; }
...
void cartridge_set_default(void) {
  ...
  DBG(("cartridge_set_default: id %d '%s'", type,
       cartfile ? cartfile : "NULL"));
...
static int cart_bin_attach(int type, const char *filename, uint8_t *rawcart) {
  ...
  log_error(LOG_DEFAULT, "cartridge_bin_attach: unsupported type (%04x)",
            (unsigned int)type);
  return -1;
}
...
int cartridge_attach_image(int type, const char *filename) {
  ...
  if (type == CARTRIDGE_CRT) {
    ...
    if (carttype == -1) {
      log_message(LOG_DEFAULT, "CART: '%s' is not a valid CRT file.",
                  abs_filename);
...
#ifdef DEBUGCART
  int delay =
      lib_unsigned_rand(1, (unsigned int)machine_get_cycles_per_frame());
#endif
#if 0
    cart_freeze_alarm_time = maincpu_clk + delay;
    alarm_set(cartridge_freeze_alarm, cart_freeze_alarm_time);
#endif
  DBG(("cartridge_trigger_freeze type:%d delay %d cycles", mem_cartridge_type,
       delay));
...
int cartridge_snapshot_write_modules(struct snapshot_s *s) {
  ...
  m = snapshot_module_create(s, SNAP_MODULE_NAME, PLUS4CART_DUMP_VER_MAJOR,
                             PLUS4CART_DUMP_VER_MINOR);
  if (m == NULL) {
    return -1;
  }
...
int cartridge_snapshot_read_modules(struct snapshot_s *s) {
  ...
  if ((vmajor != PLUS4CART_DUMP_VER_MAJOR) ||
      (vminor != PLUS4CART_DUMP_VER_MINOR)) {
    goto fail;
  }
