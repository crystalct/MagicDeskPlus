...
int cartridge_type_enabled(int type)
{
    ...
    if (type == CARTRIDGE_MAGIC_DESK) {
        int plus_enabled = 0;
        if (resources_get_int("MagicDeskPlus", &plus_enabled) == 0 && plus_enabled) {
            return 1;
        }
    }
...
/*  cart should always be detached. there is no reason for doing fancy checks
    ...
    if (slotmain) {
        if (carttype != CARTRIDGE_MAGIC_DESK && carttype != CARTRIDGE_MAGIC_DESK_16) {
            resources_set_int("MagicDeskPlus", 0);
        }
